...
def customer_query(doctype, txt, searchfield, start, page_len, filters):
	party_naming_by = frappe.defaults.get_user_default("party_naming_by")

	if party_naming_by == "Party Name":
		fields = ["name", "party_group", "territory"]
	else:
		fields = ["name", "party_name", "party_group", "territory"]

	fields = ", ".join(fields)

	return frappe.db.sql("""select %(field)s from `tabParty`
		where docstatus < 2
			and (%(key)s like "%(txt)s"
				or party_name like "%(txt)s")
			and ifnull(customer, 0) = 1
			%(mcond)s
		order by
			case when name like "%(txt)s" then 0 else 1 end,
			case when party_name like "%(txt)s" then 0 else 1 end,
			name, party_name
...
def supplier_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if party_naming_by == "Party Name":
		...
		fields = ["name", "party_name", "party_group"]
	fields = ", ".join(fields)
...
def get_project_name(doctype, txt, searchfield, start, page_len, filters):
	cond = ''
	if filters['party']:
		cond = '(`tabProject`.party = "' + filters['party'] + '" or ifnull(`tabProject`.party,"")="") and'
...
def get_delivery_notes_to_be_billed(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql("""select `tabDelivery Note`.name, `tabDelivery Note`.party_name
