...
cur_frm.cscript.onload = function(doc, cdt, cdn) {
	...
	if(doc.__islocal) {
		...
		if(last_route && last_route[0]==="Form") {
			...
			if(["Party", "Quotation", "Sales Order", "Sales Invoice", "Delivery Note",
				...
				if(refdoc.doctype == "Quotation" ? refdoc.quotation_to=="Customer" : true) {
					cur_frm.set_value("party", refdoc.party || refdoc.name);
					cur_frm.set_value("party_name", refdoc.party_name);
					if(cur_frm.doc.doctype==="Address")
						cur_frm.set_value("address_title", cur_frm.doc.party_name);
				}
			}
			if(["Supplier Quotation", "Purchase Order", "Purchase Invoice", "Purchase Receipt"]
				.indexOf(doctype)!==-1) {
				var refdoc = frappe.get_doc(doctype, docname);
				cur_frm.set_value("party", refdoc.party || refdoc.name);
				cur_frm.set_value("party_name", refdoc.party_name);
			...
			if(["Lead", "Quotation"]
				...
				if(refdoc.doctype == "Quotation" ? refdoc.quotation_to=="Lead" : true) {
					...
					cur_frm.set_value("lead_name", refdoc.party_name || refdoc.company_name || refdoc.lead_name);
					if(cur_frm.doc.doctype==="Address")
