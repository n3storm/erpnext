...
class BuyingController(StockController):
	...
	def validate(self):
		...
		if getattr(self, "party", None) and not self.party_name:
			self.party_name = frappe.db.get_value("Party", self.party, "party_name")
	...
	def set_missing_values(self, for_validate=False):
		...
		self.set_party_from_item_default()
		self.set_price_list_currency("Buying")

		# set contact and address details for supplier, if they are not mentioned
		if getattr(self, "party", None):
			self.update_if_missing(get_party_details(self.party, party_type="Supplier"))

		self.set_missing_item_details()
		if self.get("__islocal"):
			self.set_taxes("other_charges", "taxes_and_charges")

	def set_party_from_item_default(self):
		if self.meta.get_field("party") and not self.party:
