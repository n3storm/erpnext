...
def execute(filters=None):
	...
	party_map = get_party_deatils(invoice_list)
	...
	for inv in invoice_list:
		...
		row = [inv.name, inv.posting_date, inv.party, inv.party_name, inv.debit_to,
			party_map.get(inv.party), party_map.get(inv.party), inv.project_name,
...
def get_columns(invoice_list):
	...
	columns = [
		"Invoice:Link/Sales Invoice:120", "Posting Date:Date:80", "Party::120",
		"Party Name::120", "Party Account:Link/Account:120", "Party Group:LInk/Party Group:120",
...
def get_conditions(filters):
	...
	if filters.get("party"): conditions += " and party = %(party)s"
...
def get_invoices(filters):
	...
	return frappe.db.sql("""select name, posting_date, debit_to, project_name, party,
		party_name, remarks, net_total, grand_total, rounded_total, outstanding_amount
...
def get_party_deatils(invoice_list):
	party_map = {}
	parties = list(set([inv.party for inv in invoice_list]))
	for party in frappe.db.sql("""select name, territory, party_group from `tabParty`
		where name in (%s)""" % ", ".join(["%s"]*len(parties)), tuple(parties), as_dict=1):
			party_map[party.name] = party.territory
			party_map.setdefault(party.name, party)
