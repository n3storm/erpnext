...
def get_suppliers_details(filters):
	...
	for d in frappe.db.sql("""select pr.party, pri.item_code from
		...
		pri.item_code=(select name from `tabItem` where
			...
			item_supplier_map.setdefault(d.item_code, []).append(d.party)

	if party:
		for item_code, parties in item_supplier_map.items():
