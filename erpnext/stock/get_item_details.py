...
def get_pricing_rules(args_dict):
	...
	conditions = ""
	for field in ["party", "campaign"]:
			if args_dict.get(field):
				conditions += " and ifnull("+field+", '') in (%("+field+")s, '')"
			else:
				conditions += " and ifnull("+field+", '') = ''"

	for doctype in ["Party Group", "Territory"]:
...
def filter_pricing_rules(args_dict, pricing_rules, price_or_discount):
	...
	all_fields = ["item_code", "item_group", "brand", "party", "party_group", "territory",
		"campaign", "for_price_list"]

	if len(pricing_rules) > 1:
		for field_set in [["item_code", "item_group", "brand"],	["party", "party_group", "territory"]]:
