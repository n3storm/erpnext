...
class ProductionPlanningTool(Document):
	...
	def get_so_details(self, so):
		...
		so = frappe.db.sql("""select transaction_date, party, grand_total
	...
	def get_open_sales_orders(self):
		...
		if self.party:
			so_filter += ' and so.party = "' + self.party + '"'

		if self.fg_item:
			item_filter += ' and item.name = "' + self.fg_item + '"'

		open_so = frappe.db.sql("""
			select distinct so.name, so.transaction_date, so.party, so.grand_total
