...
frappe.ui.form.on_change("Opportunity", "party", function(frm) {
	erpnext.utils.get_party_details(frm) });
frappe.ui.form.on_change("Opportunity", "party_address", erpnext.utils.get_address_display);
...
erpnext.selling.Opportunity = frappe.ui.form.Controller.extend({
	onload: function() {
		if(!this.frm.doc.enquiry_from && this.frm.doc.party)
		...
		if(this.frm.doc.party && !this.frm.doc.party_name) cur_frm.cscript.party(this.frm.doc);
	...
	setup_queries: function() {
		...
		this.frm.set_query("party_address", function() {
			if(me.frm.doc.lead) return {filters: { lead: me.frm.doc.lead } };
			else if(me.frm.doc.party) return {filters: { party: me.frm.doc.party } };
		});
...
cur_frm.cscript.lead = function(doc, cdt, cdn) {
	cur_frm.toggle_display("contact_info", doc.party || doc.lead);
