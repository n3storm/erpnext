...
class Lead(SellingController):
	def onload(self):
		party = webnotes.conn.get_value("Party", {"lead": self.doc.name})
		if party:
			self.doc.fields["__is_party"] = party
	...
	def has_customer(self):
		return frappe.db.get_value("Party", {"lead": self.name})
...
@frappe.whitelist()
def make_party(source_name, target_doc=None):
	return _make_party(source_name, target_doc)

def _make_party(source_name, target_doc=None, ignore_permissions=False):
	from frappe.model.mapper import get_mapped_doc

	def set_missing_values(source, target):
		if source.company_name:
			target.party_type = "Company"
			target.party_name = source.company_name
		else:
			target.party_type = "Individual"
			target.party_name = source.lead_name

		target.party_group = frappe.db.get_default("customer_group")
		target.customer = 1
