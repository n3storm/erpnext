...
class SMSCenter(Document):
	...
	def create_receiver_list(self):
		...
		if self.send_to == 'All Customer Contact':
			where_clause = " and party = '%s'" % self.customer.replace("'", "\'") if self.customer \
				else " and party in (select name from `tabParty` where ifnull(customer, 0)=1 and docstatus<2)"

		elif self.send_to == 'All Supplier Contact':
			where_clause = " and party = '%s'" % self.supplier.replace("'", "\'") if self.supplier \
				else " and party in (select name from `tabParty` where ifnull(supplier, 0)=1 and docstatus<2)"

		elif self.send_to == 'All Sales Partner Contact':
			where_clause = " and party = '%s'" % self.sales_partner.replace("'", "\'") if self.sales_partner \
				else """ and party in (select name from `tabParty`
					where ifnull(sales_partner, 0)=1 and docstatus<2)"""
		...
		if self.send_to in ['All Contact', 'All Customer Contact', 'All Supplier Contact', 'All Sales Partner Contact']:
			rec = frappe.db.sql("""select CONCAT(ifnull(first_name,''), '', ifnull(last_name,'')),
				mobile_no from `tabContact` where ifnull(mobile_no,'')!='' and docstatus != 2 %s""",
				where_clause)
