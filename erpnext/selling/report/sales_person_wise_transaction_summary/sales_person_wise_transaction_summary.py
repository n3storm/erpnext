...
def get_columns(filters):
	...
	return [filters["doc_type"] + ":Link/" + filters["doc_type"] + ":140",
		"Customer (Party):Link/Party:140", "Territory:Link/Territory:100", "Posting Date:Date:100",
...
def get_entries(filters):
	...
	entries = frappe.db.sql("""select dt.name, dt.party, dt.territory, dt.%s,
...
def get_conditions(filters, date_field):
	...
	if filters.get("party"): conditions += " and dt.party = '%s'" % \
		filters["party"].replace("'", "\'")
